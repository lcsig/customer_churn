...
├── drift_detection/                 # Data-drift reporting
│   ├── __init__.py
│   ├── generate_drift_report.py     # Compare derived features across weeks
│   └── reports/                     # HTML reports generated by the script
│
...
`drift_detection.generate_drift_report` provides a lightweight sanity check for distributional drift across the feature set generated by `FeatureSet2`.

- Loads `data_clean_process/customer_churn_cleaned.json` and walks through four weekly cutoffs (starting Oct 29, 2018) to generate feature snapshots with active users only.
- Treats Period 1 as the reference distribution and compares Periods 2–4 via KS-test for numeric features and Total Variation Distance (TVD) for categoricals. Columns whose drift score exceeds 0.15 are flagged.
- Each comparison emits an HTML report named `drift_report_period_<n>_vs_reference.html` inside `drift_detection/reports/`; the directory is automatically created when the script runs.
- Run it via `python -m drift_detection.generate_drift_report` to regenerate the latest drift checks; the script already prints a summary of drift share and top drifted columns.

This complements the training pipeline by alerting you when the input distribution shifts significantly, helping catch upstream issues before retraining.
